...
(function ( define ) {
  ...
  define([
      'components/ui/util/math/matrix'
    ],
    function (
      Matrix
    ){

      var UIComponent = function(node, elem){
          this.node = node;
          this.elem = elem;
      ...
      UIComponent.prototype.configure = function(n){
      ...
      UIComponent.prototype.isInt = function(n){

          return Number(n) === n && n % 1 === 0;

      };

      UIComponent.prototype.isFloat = function(n){
      ...
      UIComponent.prototype.degreesToRadians = function(degrees) {

        return degrees * (Math.PI / 180);

      };

      UIComponent.prototype.transform = function(node){
        ...
        if(node.size) {
          ...
          if(node.size[0] === 1) {
            node.size[0] = parseFloat(1.0);
          }
          if(node.size[1] === 1) {
            node.size[1] = parseFloat(1.0);
          }
          ...
          if(node.size[0] === null) {
              ...
              this.elem.style.width = this.isFloat(node.size[0]) ?  node.size[0]*100+'%' : node.size[0]+'px';
          }
          if(node.size[1] === null) {
              ...
              this.elem.style.height = this.isFloat(node.size[1]) ?  node.size[1]*100+'%' : node.size[1]+'px';
      ...
      UIComponent.prototype.setPerspective = function(p){
